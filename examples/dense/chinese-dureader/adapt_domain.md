...
We use CPR-Ecommerce as an example. To adapt to an unseen domain, we unsupervisedly train a separate domain plugin model based on the corpus.
```bash
...
Now we assemble the trained domain plugin and the service model. We use the contrastively finetuned service model as an example. 
```bash
